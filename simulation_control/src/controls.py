...
def get_jacobian_and_wheel_speed(robot_data):
    ...
    if(robot_data.robot_type == 1):
        #rospy.loginfo('Selected Differential Drive Mobile Robot')
        ...
        #rospy.loginfo('Selected Mecanum Drive Mobile Robot')
        ...
        #rospy.loginfo('Selected Omnidirectional Drive Mobile Robot')
        l_omni = robot_data.width/2

        omni_J_1 = np.array([[math.sin(math.pi/3),-1/2,-l_omni],
                             [0,1,-l_omni],
                             [-math.sin(math.pi/3),-1/2,-l_omni]])
...
def calculate_odom_values(robot_type, jacobian, wheel_speed, curr_orientation):
    ...
    if(robot_type == 2):
        j_1p = np.dot(np.linalg.inv(np.dot(transpose(jacobian),jacobian)),transpose(jacobian))
        temp_values = np.dot(j_1p,wheel_speed)
    elif((robot_type == 1) or (robot_type == 3)):
